...
use Kommandhub\ShippingSW\Webhook\Service\WebhookEventFactory;
use Kommandhub\ShippingSW\Webhook\Service\WebhookSignatureValidator;
...
/**
 * Public endpoint Shipping posts to.
 *
...
class WebhookController extends StorefrontController
{
    ...
    #[Route(
        ...
        name: 'kommandhub_shipping_webhook',
        methods: ['POST'],
    )]
    public function handle(Request $request): Response
    {
        ...
        $eventType = is_string($payload['event'] ?? null) ? $payload['event'] : '';
        $event = $this->eventFactory->create($eventType, $payload, $salesChannelId);

        if ($event === null) {
            $this->logger->info('Ignoring unhandled Shipping webhook', [
                'event' => $eventType,
